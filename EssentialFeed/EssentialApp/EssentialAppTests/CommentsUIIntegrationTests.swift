...
class CommentsUIIntegrationTests: XCTestCase {
	...
	func test_loadCommentsActions_requestCommentsFromLoader() throws {
		...
		XCTAssertEqual(loader.loadCommentsCallCount, 1, "Expected no request until previous request finishes")
	...
	func test_loadingCommentsIndicator_isVisibleWhileLoadingComments() throws {
		let (sut, loader) = makeSUT()
	...
	func assertThat(_ sut: ListViewController, isRendering comments: [ImageComment], file: StaticString = #filePath, line: UInt = #line) {
		...
		viewModel.comments.enumerated().forEach { index, comment in
			...
			XCTAssertEqual(sut.commentDate(at: index), comment.date, "created at \(index)", file: file, line: line)
